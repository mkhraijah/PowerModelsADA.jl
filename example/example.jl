...
case_path = "test/data/case_ACTIVSg10k.m"
...
data_area = PMADA.solve_dopf_admm(data, model_type, optimizer, tol=tol, max_iteration=max_iteration, verbose = true, alpha=100000);
...
error_app = PMADA.compare_solution(data, data_area, model_type, optimizer)
data_area = PMADA.solve_dopf_atc(data, model_type, optimizer, tol=tol, max_iteration=max_iteration, verbose = true, alpha=1.05);
error_atc = PMADA.compare_solution(data, data_area, model_type, optimizer)


data_coordinator, data_area = PMADA.solve_dopf_admm_coordinated(data, model_type, optimizer; max_iteration=max_iteration, alpha = 1000);
error_admm = PMADA.compare_solution(data, data_area, model_type, optimizer)
data_coordinator, data_area = PMADA.solve_dopf_atc_coordinated(data, model_type, optimizer; max_iteration=max_iteration, alpha = 1.05);
error_atc = PMADA.compare_solution(data, data_area, model_type, optimizer)